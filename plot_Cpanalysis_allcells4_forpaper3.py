...
##BELOW ARE THE EXTRA DATA PLOTS
    ...
    axl[0].set_xlabel('Phase\nfraction', fontsize=11)
    axl[0].set_xticks([0, 0.3, 0.6, 0.9])
    pylab.subplots_adjust(left=.35, right=.68, top=0.95, hspace=0.01, bottom=.21)
    pylab.savefig(os.path.join(os.path.join(savef, 'cell%02d' %selectcell), 'PhaseConcstack_cell%02d.eps' %(selectcell)))
...
#    pylab.savefig(os.path.join(os.path.join(savef, 'cell%02d' %selectcell), 'CpRegionLegend_cell%02d_%s.png' %(selectcell, metadict['name'])), dpi=600)
    ...
    pylab.subplots_adjust(left=0.27, right=.50, top=0.95, hspace=0.01, bottom=.26)
    pylab.savefig(os.path.join(os.path.join(savef, 'cell%02d' %selectcell), 'Cool320Cstack_cell%02d.eps' %(selectcell)))
