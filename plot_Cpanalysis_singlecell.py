...
pylab.savefig(os.path.join(os.path.join(savef, 'cell%02d' %selectcell), 'Cpstack_cell%02d_%s.png' %(selectcell, metadict['name'])))
    ...
    if pcal!=cal:#if previous scan was in same heat# as scan then there was no xrd in between
        for d in xrddictlist:
            if d['name']==pcal:#use xrd that happened after the prev scan
                c=numpy.float32([d['amfrac'], d['fccfrac'], d['othfrac']])
                c=c/c.sum()
                print c
                break
    phasecomps+=[c]
...
for count, i in  enumerate(sortinds):
    ...
    axl[count].barh(range(len(arr)),arr,color=['r','g','b'],height=1)
for ax in axl:
