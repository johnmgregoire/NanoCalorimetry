...
for i, (e, h, l) in enumerate(ehl):
    ...
    if 1:
        time=hpsdl[4]['cycletime'][0, :20000]
        dT=hpsdl[4]['sampleheatrate'][0, :20000]
        pylab.plot(time, dT, label=l)
        pylab.xlabel('time (s)')
        pylab.ylabel('dT/dt (K/s)')
        pylab.legend(loc=0)
    if 0:
        T=hpsdl[4]['sampletemperature'][0, :20000]
        ppr=hpsdl[4]['samplepowerperrate'][0, :20000]
        if 1:
...
def extremesmooth(x, binn=50, SGpts=200, SGorder=3):
