...
fom=numpy.zeros(25, dtype='float32')
metadictlist=[]
allsegdict=[]
for count, ed in enumerate(expdicts):
    ...
    for hpp in hppaths:
        ...
        if not hpsdl is None:
            ...
            fom[cell-1]=max(maxT, fom[cell-1])
            #fom[cell-1]+=timeabove
tcrcalc=lambda Tm:(Tm>1410. and (6.7e-7*(Tm-1410.), ) or (0, ))[0]+9.47e-4
fomlabel='maxT'
pylab.subplot(211)
pylab.plot(numpy.arange(25)+1, fom, '.')
pylab.ylabel(fomlabel)
pylab.xlabel('cell')
pylab.subplot(212)
pylab.plot(fom[tcrcells-1],tcr[tcrcells-1],  '.')
temp=numpy.linspace(1300, 1600, 300)
pylab.plot(temp, [tcrcalc(t) for t in temp], 'r-')
pylab.xlabel(fomlabel)
pylab.ylabel('tcr')
