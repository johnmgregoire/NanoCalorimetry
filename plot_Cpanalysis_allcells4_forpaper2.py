...
#pylab.rcParams['font.size']=fs10
#pylab.rcParams['xtick.labelsize']=fs10
#pylab.rcParams['ytick.labelsize']=fs10
pylab.rcParams['xtick.major.size']=9
pylab.rcParams['ytick.major.size']=9
pylab.rcParams['xtick.major.pad']=17
pylab.rcParams['ytick.major.pad']=17
pylab.rcParams['axes.linewidth']=lw
...
#Cp plots
        axl[count].plot(T, mult*C, '-', color=cols[count], lw=lw, label=metadict['name'])
        ...
        for regind, (enth, Tp, Cp, Tmean, i, j) in enumerate(zip(metadict['Cpregions_enthalpy'], metadict['Cpregions_T_Cmax'], metadict['Cpregions_Cmax'], metadict['Cpregions_Tweightedmean'], metadict['Cpregions_cycindstart'], metadict['Cpregions_cycindstop'])):
            if regind in rxnindlist:
                ...
                continue
            #axl[count].fill(T[i:j], mult*C[i:j], color='w', hatch=hatch, lw=3., alpha=1.)
            axl[count].fill(T[i:j], mult*C[i:j], color=col, alpha=0.3)
            
            #axl[count].plot(Tp, mult*Cp, 'kx')
            #axl[count].plot(Tmean, 0, 'k*')
        if count==4:
            pylab.annotate('glass trans.', (200, 1.1), (220, 2.2), arrowprops=dict(facecolor='black', shrink=0.05, headwidth=15), horizontalalignment='center', verticalalignment='bottom', fontsize=fs9)
            pylab.annotate(' crystalliz.', (365, -.7), (410, -1.), arrowprops=dict(facecolor='black', shrink=0.05, headwidth=15), horizontalalignment='left', verticalalignment='center', fontsize=fs9)
            pylab.annotate('melting', (440, 1.5), (480, 2.6), arrowprops=dict(facecolor='black', shrink=0.05, headwidth=15), horizontalalignment='center', verticalalignment='bottom', fontsize=fs9)
    ...
    for ax in axl:
        ...
        for tlabel in ax.xaxis.get_ticklabels():
            tlabel.set_fontsize(fs8)
        for tlabel in ax.yaxis.get_ticklabels():
            tlabel.set_fontsize(fs8)
        #set_ticksize(ax, size=ts)
    
#        ax.axhline(linewidth=axlw)
#        ax.axvline(linewidth=axlw)
    axl[2].set_ylabel(r'Calorimetric signal ($\mu$J/K)', fontsize=fs11)
    axl[0].set_xlabel('Temperature (C)', fontsize=fs11)
    headname='Cpstack'
###****    
##Cp fit plots
...
#****       
    pylab.savefig(os.path.join(os.path.join(savef, 'cell%02d' %selectcell), '%s_cell%02d_%s.png' %(headname, selectcell, namestack)), dpi=dpi)
...
###BELOW ARE THE EXTRA DATA PLOTS
    ...
    ##barplot
    pylab.figure(figsize=figsize2, dpi=dpi)
    ...
    for count, i in  enumerate(sortinds):
        ...
        if count==2:
            for text_lab, col_lab, x_lab, y_lab in zip(['amorph.', 'intermet.', 'fcc'], ['y','g','r'], [arr.max()*1.07]*3, [.3, 1.5, 2.7]):
                axl[count].text(x_lab, y_lab, text_lab, color=col_lab, fontdict={'fontsize':fs9},va='center',ha='left', fontsize=fs9)
        for tlabel in axl[count].xaxis.get_ticklabels():
            tlabel.set_fontsize(fs9)
    ...
    axl[0].set_xlabel('phase fraction', fontsize=fs11)
    axl[0].set_xticks([0., .3, .6, .9])    
    pylab.subplots_adjust(left=.25, right=.68, top=0.95, hspace=0.01, bottom=.17)
    pylab.savefig(os.path.join(os.path.join(savef, 'cell%02d' %selectcell), 'PhaseConcstack_cell%02d.png' %(selectcell)), dpi=dpi)
...
#    pylab.savefig(os.path.join(os.path.join(savef, 'cell%02d' %selectcell), 'CpRegionLegend_cell%02d_%s.png' %(selectcell, metadict['name'])), dpi=600)
    ...
    #plot cooling rates
    pylab.figure(figsize=figsize3)
    for count, i in enumerate(sortinds):
        if count==0:
            pass
        else:
            pylab.semilogx(numpy.abs(metadictlist[i]['prevcoolrate_320C']), count, 'o', color=cols[count], markersize=24)
    make_ticklabels_invisible(pylab.gca(), x=False)
    pylab.xlabel('Cool. rate (K/s)', fontsize=fs11)
    ...
    pylab.savefig(os.path.join(os.path.join(savef, 'cell%02d' %selectcell), 'Cool320Cstack_cell%02d.png' %(selectcell)), dpi=dpi)
