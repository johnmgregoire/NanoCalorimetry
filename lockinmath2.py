...
def lia_ampphase(x, ptspercyc, ncyclewin=1., returnphase=True, pad=True):
    ...
    if returnphase:
        phase=numpy.array([numpy.arctan(s[i:i+npts].sum()/c[i:i+npts].sum()) for i in numpy.arange(len(x)-npts)])
    if pad:
...
def liaharmonic_relphase(x, phaserefdata, ptspercyc, ncyclewin_1w=2., ncyclewin_nw=6., harmonic=3., phaseshift=0., pad=True):
    ...
    ph1w=numpy.array([numpy.arctan(s[i:i+npts].sum()/c[i:i+npts].sum()) for i in numpy.arange(len(phaserefdata)-npts)])
